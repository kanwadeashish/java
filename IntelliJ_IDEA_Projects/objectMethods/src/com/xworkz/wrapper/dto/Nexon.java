...
public class Nexon {
    ...
    public Nexon(String carName, int carPrice, byte carNumber, short numberPlate, long serialNumber, double engineLitres, float tax, boolean isCarHybrid, char carRating) {
    ...
    public String toString(){
        return "Nexon[carName ="+carName+" carPrice ="+carPrice+" carNumber ="+carNumber+" numberPlate ="+numberPlate+" serialNumber ="+serialNumber+" enginelitres ="+engineLitres+" tax ="+tax+" isCarHybrid ="+isCarHybrid+" carRating ="+carRating+"]";
    }
    ...
    public boolean equals(Object obj) {
        ...
        Nexon nexon = (Nexon) obj;

        if (carPrice != nexon.carPrice) return false;
        if (carNumber != nexon.carNumber) return false;
        if (numberPlate != nexon.numberPlate) return false;
        if (serialNumber != nexon.serialNumber) return false;
        if (Double.compare(nexon.engineLitres, engineLitres) != 0) return false;
        if (Float.compare(nexon.tax, tax) != 0) return false;
        if (isCarHybrid != nexon.isCarHybrid) return false;
        if (carRating != nexon.carRating) return false;
        return carName != null ? carName.equals(nexon.carName) : nexon.carName == null;
    }
